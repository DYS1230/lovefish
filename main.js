...
var color = ['red', 'yellow', 'green', 'teal', 'blue', 'purple', 'orange'];  //魔性海葵颜色与波纹颜色
...
function loop() {
	...
	window.requestAnimFrame(loop);  //循环函数，html5新的循环函数，在commonFunctions里面
	var now = Date.now();            // 循环了一次，现在的时间
	deltaTime = now - lastTime;        // 两次循环间隔时间
	lastTime = now;                  // 把上一次时间定义为现在循环的时间
	
	contextBack.clearRect(0, 0, drawWidth, drawHeight); //此句话要也行不要也行，因为下面那句话就把自前的画面覆盖了
	contextBack.drawImage(backgroundPic, 0, 0, drawWidth, drawHeight);             //绘制背景
	ane.drawAne();               //绘制海葵
